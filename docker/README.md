...
Images are based on [glibc-busybox](https://hub.docker.com/_/busybox/)  or [Debian](https://hub.docker.com/_/debian/) and include only `hadolint` static binary.
...
For each image, there is a Debian alternative with `-debian` suffix in a tag which can be useful in some corner cases, eg. UTF-8 symbols in `Dockerfile`.
