...
spec = do
  ...
  describe "DL2036 - Use only an allowed registry in the FROM image" $ do
    ...
    it "warn on non-allowed registry" $ do
      ...
      let ?rulesConfig = Hadolint.Process.RulesConfig ["docker.io"] Map.empty (Just False)
    ...
    it "does not warn on allowed registries" $ do
      ...
      let ?rulesConfig = Hadolint.Process.RulesConfig ["x.com", "random.com"] Map.empty (Just False)
      ruleCatchesNot "DL3026" $ Text.unlines dockerFile
    ...
    it "allows using previous stages" $ do
      ...
      let ?rulesConfig = Hadolint.Process.RulesConfig ["random.com"] Map.empty (Just False)
      ruleCatchesNot "DL3026" $ Text.unlines dockerFile
